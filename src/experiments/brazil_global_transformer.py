"""
Brazil — Global Transformer experiment.
"""
import torch
...
def run(cfg: ExperimentConfig = None):
    if cfg is None:
        cfg = ExperimentConfig(
            name="brazil_global_transformer",
            data=DataConfig(country="brazil", input_window=14, output_window=1),
            model=ModelConfig(
                model_type="global_transformer",
                hidden_dim=64, nhead=4, num_layers=1,
                ff_dropout=0.2, gat_heads=2,
            ),
            train=TrainConfig(epochs=50, batch_size=32, lr=1e-3, scheduler_step=10, scheduler_gamma=0.95),
        )
